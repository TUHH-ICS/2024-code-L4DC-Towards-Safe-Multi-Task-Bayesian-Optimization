...
from utils.utils import concat_data, sample_from_task
...
if len(sys.argv) > 1:
    add_arg = sys.argv[1]
    if "trunc" in add_arg:
        utils.get_robust_gp.change_truncate(True)
...
for i in range(X_init.size(0)):
    ...
    gp = utils.utils.build_gp(
        (train_inputs, train_tasks), train_targets
    )  # build GP model
    bo = BayesianOptimization(
        obj, tasks, norm_bounds, T, [1, 15, 15]
    ...
    output_path = output_dir / f"num_laser{num}.obj"
