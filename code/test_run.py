...
from utils.utils import concat_data, plot_post, plot_post2D, sample_from_task
...
G = [
    build_laser_model(num, disturbance=0.0 if i == 0 else disturbance)
...
bo = BayesianOptimization(
    obj, tasks, norm_bounds, T, [1, 15, 15], boundary_T=-15
)  # Define BO object 1 sample from main, 15 from supplementary tasks

pot_start, _, pot_start_targets = sample_from_task(obj, [0], norm_bounds, n=20)
ind = (
    pot_start_targets[pot_start_targets.squeeze() >= bo._det_threshold(pot_start)]
    .squeeze()
    .argmin()
)

norm_x0 = pot_start[pot_start_targets.squeeze() >= bo._det_threshold(pot_start), ...][
    ind, ...
].view(1, d)
...
for i in range(1, num_tsk):
    ...
    train_inputs, train_tasks, train_targets = concat_data(
        (x, t, y), (train_inputs, train_tasks, train_targets)
    )
...
for i in range(nruns):
    ...
    if i % 2 == 0:
        ...
        robust_gp, sqrtbeta, gamma = utils.get_robust_gp.getboundinggp(
            sample_models, gp
        )
        if d == 1:
