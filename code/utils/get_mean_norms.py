...
def get_mean_norms(model0, model, lambda_):
    ...
    IK01 = torch.linalg.solve(
        model0.likelihood(S0).covariance_matrix, torch.eye(K0.size(0))
    )
    S1 = model(train_inputs)
    K1 = S1.covariance_matrix
    IK11 = torch.linalg.solve(
        model.likelihood(S1).covariance_matrix, torch.eye(K1.size(0))
    )
    return torch.sqrt(
        y.T
        @ (lambda_**2 * IK01 @ K0 @ IK01 - 2 * IK01 @ K1 @ IK11 + IK11 @ K1 @ IK11)
        @ y
    )
