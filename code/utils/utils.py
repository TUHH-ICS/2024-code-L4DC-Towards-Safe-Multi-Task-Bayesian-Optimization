...
def build_gp(train_inputs, train_targets):
    ...
    gp.mean_module._constant_closure(
        gp.mean_module, torch.tensor([-30.0])
    )  # const mean
    gp.covar_module._set_outputscale(torch.tensor(15**2))  # sigma_f
    gp.likelihood.noise = torch.tensor([0.1])  # sigma_n
    if d == 1:
        ...
        gp.covar_module.base_kernel._set_lengthscale(
            torch.tensor([ell, 1.5]).repeat(1, int(d / 2))
        )  # lengthscale
    return gp
...
def plot_post(gp, task, test_x, sqrtbeta=None):
    ...
    with torch.no_grad():
        ...
        if sqrtbeta is not None:
            upper = (upper.squeeze() - ymean) / 2 * sqrtbeta + ymean
            lower = (lower.squeeze() - ymean) / 2 * sqrtbeta + ymean
    ...
    i = i.squeeze()
    y = gp.train_targets
    ax.fill_between(test_x, upper.squeeze(), lower.squeeze(), alpha=0.2, color="b")
...
def plot_post2D(gp, task, bounds, samps=101, sqrtbeta=None):
    ...
    sqrtbeta = 2 if sqrtbeta is None else sqrtbeta
    ...
    ax.plot_wireframe(
        X.cpu().detach().numpy(),
        Y.cpu().detach().numpy(),
        (ymean + sqrtbeta * torch.sqrt(yvar)).cpu().detach().numpy(),
        color="r",
        rstride=10,
        cstride=10,
    )
...
def sample_from_task(obj, tasks, bounds, n=5, data=None):
    for i in tasks:
        ...
        data = concat_data(
            (X_init, i * torch.ones(X_init.size(0), 1), obj[i](X_init)), data
        )
    return data
